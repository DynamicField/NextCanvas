...
namespace NextCanvas.Controls
{
    ...
    public class NextInkCanvas : InkCanvas
    {
        ...
        public static readonly DependencyProperty EraserShapeDPProperty = DependencyProperty.Register(
            "EraserShapeDP",
            typeof(StylusShape),
            typeof(NextInkCanvas),
            new PropertyMetadata(
                (sender, e) =>
                {
                    ((NextInkCanvas) sender).EraserShape = e.NewValue as StylusShape ??
        ...
        public void SelectChildren(object dataContext)
        {
            Select(
                new[]
                {
                    GetElementFromDataContext(this, dataContext)
                });
        }

        protected override void OnSelectionChanged(EventArgs e)
        {
            ReadOnlyCollection<UIElement> elements = GetSelectedElements();
            if (elements.Count == 1)
            {
                elements[0].Focus();
        ...
        private static void CanExecuteCommand(object sender, CanExecuteRoutedEventArgs e)
        {
            ...
            if (e.Command == ApplicationCommands.Delete)
            {
                e.CanExecute = canvas.GetSelectedElements()
                                     .Any() || canvas.GetSelectedStrokes()
                                                     .Any();
            }
        ...
        {
            StrokeCollection list = GetSelectedStrokes();
            Strokes.Remove(list);
            ReadOnlyCollection<UIElement> elements = GetSelectedElements();
            if (elements.Any())
            {
